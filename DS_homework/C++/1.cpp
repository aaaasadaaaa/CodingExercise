...
int main(int argc, char const *argv[]){
    using namespace std;
    u = argv[2].erase(argv[2].end() - 1);
    int simb = compare(argv[1]);            //judge whether the form is shanghaitech or gregorian
    if (simb == 1)
        shanghaitech(argv[1],u,argv[3]) ;      // change the form from geogorian to shanghaitech
    else
        Gregorian(argv[1],u,argv[3]) ;        // change the form from shanghaitech to gregorian
}
...
int shanghaitech(string month, string date, string year){
    ...
    month_name = judge_month_name(start_day[0],start_day[2]);
