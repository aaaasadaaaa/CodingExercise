...
void transpose_blocking(int n, int blocksize, int *dst, int *src) {
    ...
    /*int submatrix = n - n % blocksize;
    int num_block = submatrix / blocksize;
    for (int u = 0; u < num_block; u++) {
        for (int x = 0; x < blocksize; x++) {
            for (int y = 0; y < blocksize; y++) {
                dst[y + u * blocksize + x * n] = src[x  + (y + u * blocksize) * n];
            }
        }
    }
    for (int z = n - submatrix; z < n; z++ ){
        for (int p = n - submatrix; p < n; p++ ) {
            dst[z + p * n] = src[p + z * n];
        }
    }*/
    // YOUR CODE HERE
